...
Receive WhatsApp Cloud API events through an ngrok HTTPS URL, verify Meta's callback, parse incoming text messages, safely log receipt, and prevent duplicate intake with the existing SQL database. A local CLI can explicitly send text or template messages through the existing WhatsApp service. Optional automation generates short OpenAI replies through the existing inbox worker and durable outbox.

```text
Meta WhatsApp Cloud API -> Meta webhook -> ngrok HTTPS
  -> Express /webhook -> signature validation -> parser -> durable SQL inbox -> HTTP 200
```

**`AUTOMATION_ENABLED=false` is the default.** With `AI_PROVIDER=openai`, explicitly enabling automation starts customer conversations and the configured boss lead workflow. Leaving `AI_PROVIDER` blank retains the earlier fixed reply. Zoho and boss approval remain disconnected; their prior implementation is preserved in [the deferred automation reference](docs/automation-reference.md).
...
| `OPENAI_API_KEY`, `OPENAI_MODEL` | Required for OpenAI generation; supplied by the private environment, never source code. Missing/invalid AI configuration fails a worker job safely without breaking webhook receipt. |
...
| `DATABASE_URL` | Blank uses persistent local SQLite at `file:./data/messages.sqlite`. Existing production validation requires PostgreSQL. |
...
Zoho settings are not needed. Existing AI settings serve both conversations and boss extraction. Keep automation disabled until you intentionally enable it. Existing shell environment variables override `.env`; restart the backend after configuration changes. Legacy `WHATSAPP_APP_SECRET` and `WHATSAPP_API_VERSION` aliases remain supported, with the `META_*` names preferred.
...
The backend also supports one Mongoose connection configured by `MONGODB_URI`. This does not replace `DATABASE_URL`: the existing SQLite/PostgreSQL store still owns WhatsApp messages, drafts, leads and the outbox. No MongoDB Lead schema, collection creation or data migration is included.
...
`src/config/db.js` reuses a single Mongoose connection and coalesces simultaneous initialization. Startup awaits it before HTTP listening when a URI is configured. Initial connection failure stops startup with a fixed message without printing the URI or password. The initial server-selection timeout is 30 seconds, and shutdown/startup-failure cleanup releases MongoDB alongside the existing SQL store. No extra connection is made by the Express routes.

When the URI is blank, startup reports `MongoDB not configured. Set MONGODB_URI in .env to enable Atlas.` and preserves the existing backend behavior. With a valid URI, the existing JSON startup logs contain:
...
Restart using `npm.cmd start` after changing the URI. `/health` and `/ready` keep their existing API behavior; `/ready` checks the SQL-backed WhatsApp store and is not a MongoDB health check. Automated MongoDB tests mock the connection and never contact Atlas. A real Atlas connection can only be verified after you supply the URI.
...
Startup initializes database migrations automatically. Use `npm.cmd run dev` for source-change restarts. Stop the server with `Ctrl+C`. In a second terminal:
...
`GET /ready` checks database availability and reports whether automation is `disabled` or `enabled`; it does not test Meta connectivity. The existing `GET /api/health` alias is retained.
...
The smoke test creates temporary in-memory credentials and a separate temporary SQLite database, starts the actual server on loopback port 5000, checks health, GET verification, signed POST receipt, duplicate delivery, and the safe receipt log, then shuts it down. It does not need real Meta credentials or modify `.env`. Stop your manually started server first if it already occupies that port, or use `$env:SMOKE_PORT = '5050'` before the command to test on another free port. Afterwards, use `Remove-Item Env:SMOKE_PORT` to restore the default.

For exact GET and signed POST commands against a server you started, use [local webhook testing](docs/webhook-local-testing.md) and the [synthetic sample payload](docs/samples/whatsapp-text.json). The automated suite mocks external APIs. SQLite tests use isolated databases; optional PostgreSQL tests require a dedicated `TEST_DATABASE_URL` and otherwise skip.
...
Fill the API key privately. The model is selected exclusively through `OPENAI_MODEL`, with no runtime fallback. The example uses [GPT-6 Astra](https://developers.openai.com/api/docs/models/gpt-6-astra) through the existing Axios Responses integration, with `reasoning.effort=low`. No OpenAI SDK installation or dependency upgrade is needed. `AI_MAX_OUTPUT_TOKENS` includes reasoning and answer tokens; the configured cap and validation limits remain unchanged. Set the master switch to `true` only when ready, then restart the same backend. See the [Astra migration report](docs/openai-astra-migration.md) for the inspected call sites, preserved lead contracts and validation. No Meta webhook change, second server, Redis, Zoho, lead creation, or boss approval is required.
...
| Symptom | Check |
| --- | --- |
| Startup configuration error | Set `WEBHOOK_VERIFY_TOKEN`; keep `AUTOMATION_ENABLED=false`. Production additionally requires an app secret, strong token, and PostgreSQL. Shell variables override `.env`. |
...
| POST `200`, no receipt log | Check `LOG_LEVEL=info`, text type, required message fields, number-ID/sender filters, and whether the ID was already stored. Status events, unsupported types, or irrelevant structures are intentionally ignored. |
...
| POST `503` / readiness `503` | Database URL, connectivity, migrations, permissions, or capacity. Retry after fixing storage. |
