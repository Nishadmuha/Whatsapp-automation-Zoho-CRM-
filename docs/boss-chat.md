...
- `BOSS_REPLY_QUIET_MS` controls that pause (default `5000`, allowed range `1`–`60000` milliseconds). The automatic per-message “Processing the lead” acknowledgement is disabled. Superseded unsent prompts remain in the outbox as `CANCELLED`; errors and explicit save/discard replies remain deliverable. Customer replies and the worker bill workflow retain their existing behavior.
