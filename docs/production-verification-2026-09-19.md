...
Historical verification snapshot. For current minimum permissions and results, see [the subsequent OAuth scope audit](zoho-oauth-scopes.md). Chart of Accounts/account-ID setup below is no longer part of the workflow.
...
Live read-only Books checks succeeded: OAuth refresh, currency settings (AED present), and tax settings. The existing configuration recognizes **+971568556901** as an authorized and allowed Books worker, not a boss. An earlier Chart of Accounts GET returned HTTP 401/provider code 57. That obsolete integration has since been removed; no accountants scope or fixed expense-account ID is required.
...
5. **SAVE:** only `SAVE` or `1` authorizes creation. YES/NO and arbitrary text do not. Validation checks vendor, number, dates, currency, total, item quantities/rates, configured currency/tax mappings, and arithmetic. Ambiguous vendor matches and incomplete duplicate searches block creation.
...
14. **Remaining issues at that verification:** restart/redeploy the backend; perform an operator-supervised live send/SAVE only with an approved bill. Live CRM attachment visibility and actual WhatsApp delivery still need verification. The historical full-suite failure was `test/leadsDashboard.test.js:78`; see the subsequent audit for current results. Non-Latin PDF text needs a suitable font via `BILL_PDF_FONT_PATH`; otherwise generation fails explicitly. Discounts, mixed/ambiguous tax allocations, and missing accounting details require correction rather than inferred values. Existing uncertain creation/delivery records require manual reconciliation. Chart of Accounts access is not a remaining blocker.
...
1. Verify the minimum OAuth permissions in [the scope guide](zoho-oauth-scopes.md). Do not configure a fixed expense-account ID or authorize Chart of Accounts for this workflow.
...
The implementation reads saved records and maps configured currency/tax IDs using the [Zoho Books bills API](https://www.zoho.com/books/api/v3/bills/), [currency API](https://www.zoho.com/books/api/v3/currency/) and [tax API](https://www.zoho.com/books/api/v3/taxes/). Document delivery follows [Meta's document-message API](https://www.postman.com/meta/whatsapp-business-platform/request/zjtbdpz/send-document-message-by-id). The generated copy uses [PDFKit](https://pdfkit.org/docs/getting_started.html).
