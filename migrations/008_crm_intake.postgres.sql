...
CREATE TABLE crm_read_state (sender_phone TEXT PRIMARY KEY,read_at TIMESTAMPTZ NOT NULL);
...
CREATE TABLE lead_groups (
  ...
  last_message_at TIMESTAMPTZ NOT NULL,created_at TIMESTAMPTZ NOT NULL
);
...
CREATE TABLE lead_messages (
  ...
  extracted INTEGER NOT NULL DEFAULT 0,created_at TIMESTAMPTZ NOT NULL
);
...
CREATE TABLE lead_attachments (
  ...
  size_bytes INTEGER,sha256 TEXT,created_at TIMESTAMPTZ NOT NULL
);
CREATE INDEX lead_attachments_lead ON lead_attachments(lead_id,created_at,id);
CREATE TABLE crm_greetings (sender_phone TEXT PRIMARY KEY,message_id TEXT NOT NULL REFERENCES whatsapp_messages(whatsapp_message_id),created_at TIMESTAMPTZ NOT NULL);
